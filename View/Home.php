<?php
  ...
  if(isset($_GET['addNew'])){
    ...
    $sql = " INSERT INTO `list`(`id`, `list`) VALUES ('$id','$value') ";
              ...
              <th scope="col"></th>
                ...
                <tbody>
                  ...
                  $i=0;
                      ...
                      $sql = "Select list from list where id={$_SESSION['id']}";
                      $result = mysqli_query($conn, $sql);
                      if(mysqli_num_rows($result) > 0){
                          while($row = $result->fetch_assoc()){
                              echo "<tr id='{$i}'> <td class='row-data'> {$row["list"]} </td> <td> <button type='button' class='btn btn-outline-danger fw-bolder px-3 py-1 rounded-1' data-bs-toggle='modal' data-bs-target='#exampleModal' onClick='myBooking()'>Delete</button> </td></tr>";
                            $i++;
                          }
                      }
